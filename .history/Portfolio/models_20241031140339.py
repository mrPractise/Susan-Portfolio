...
class Event(models.Model):
    ...
    def is_sold_out(self):
        ...
        if not ticket_types.exists():
            return False
        return all(tt.is_sold_out for tt in ticket_types)
...
class TicketType(models.Model):
    ...
    def clean(self):
        ...
        # Check if total capacity of all ticket types exceeds event capacity
        if self.event_id:
    ...
    def save(self, *args, **kwargs):
        # Set initial available tickets if not set
...
class Booking(models.Model):
    ...
    def clean(self):
        # Validate total amount matches ticket selections
...
class BookingTicket(models.Model):
    ...
    ticket_type = models.ForeignKey(
        TicketType, 
        on_delete=models.PROTECT,  # Prevent deletion of ticket types with bookings
        related_name='bookings'
    )
    ...
    def clean(self):
        if not self.id:  # New booking ticket
            ...
            # Validate against minimum/maximum purchase limits
    ...
    def save(self, *args, **kwargs):
        # For new booking tickets, set the unit price from ticket type
        if not self.id:
