    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return redirect('contact')  # Redirect to contact page with success message
                
            except Exception as e:
                logger.error(f"Error in contact form submission: {str(e)}")
                messages.error(request, 'Sorry, there was an error sending your message. Please try again.')
        else:
            # Form is invalid, messages will be displayed in the template
            messages.error(request, 'Please correct the errors below.')
    else:
        form = ContactForm()  # GET request, create a new form instance
