...
function toggleShareMenu(button) {
  ...
  const description = eventCard.querySelector(
    ".event-description p"
  ).textContent;
  const venue = eventCard.querySelector(
    ".event-meta span:first-child"
  ).textContent;
  const time = eventCard.querySelector(
  ...
  // Generate menu items
  shareMenu.innerHTML = generateShareMenuItems(
    title,
    description,
    venue,
    time,
  ...
  document.getElementById("shareOverlay").classList.toggle("active");
}



function generateShareMenuItems(
  title,
  description,
  venue,
  time,
...
function handleShare(shareMenuItem) {
  ...
  if (
    ...
    try {
      navigator.share({
        title: shareMenuItem.closest(".event-card").querySelector("h3")
          .textContent,
        text: shareMenuItem
          .closest(".event-card")
          .querySelector(".event-description p").textContent,
