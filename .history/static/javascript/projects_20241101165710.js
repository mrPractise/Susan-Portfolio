...
class ProjectGallery {
  constructor() {
    ...
    if (GalleryInstance.instance) {
      return GalleryInstance.instance;
    }
    ...
    GalleryInstance.instance = this;
    this.initializeGallery();
  }

  initializeGallery() {
    // Prevent double initialization
    if (GalleryInstance.initialized) {
      return;
    }
    ...
    GalleryInstance.initialized = true;
  }
...
if (!window.galleryInitialized) {
  ...
  window.addEventListener(
    "DOMContentLoaded",
    () => {
      console.log("Initial gallery initialization");
      window.gallery = new ProjectGallery();
    },
    { once: true }
  ); // Ensure the event listener only fires once
}
