...
function toggleShareMenu(button) {
    ...
    // Update or create share menu
    const shareMenu = button.nextElementSibling || createShareMenu();
...
function createShareMenu() {
  ...
  button.parentNode.appendChild(menu); // Append to the event card
  return menu;
}
...
async function handleShare(shareMenuItem) {
    ...
    if (navigator.share && (platform === 'whatsapp' || platform === 'facebook' || platform === 'instagram' || platform === 'tiktok')) {
        try {
            navigator.share({
