    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          // Add a small delay between elements for a staggered effect
...
const initContact = () => {
  ...
  // Set up different delays for staggered animation
  if (contactInfo) {
    contactInfo.dataset.delay = "0";
  }
  if (contactForm) {
    contactForm.dataset.delay = "150"; // Slight delay for form
  }

  // Create a separate observer for contact section with different options
