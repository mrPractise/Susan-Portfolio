class VideoPlayer {
  ...
  openModal() {
    // Clone video element and controls for modal
    const videoContainer = this.container.cloneNode(true);
    const videoClone = videoContainer.querySelector("video");

    this.modalContainer.innerHTML = "";
    this.modalContainer.appendChild(videoContainer);
    ...
    // Transfer current time from original video
    videoClone.currentTime = this.video.currentTime;
  }

  closeModal() {
    const modalVideo = this.modalContainer.querySelector("video");
    if (modalVideo) {
      modalVideo.pause();
    }
