...
def validate_video_file(value):
    ...
    if not ext.lower() in valid_extensions:
        raise ValidationError('Unsupported file type. Please use MP4, WebM, or OGG format.')
class GalleryCategory(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.is_default:
            ...
            GalleryCategory.objects.filter(is_default=True).update(is_default=False)
        super().save(*args, **kwargs)
...
class MediaItem(models.Model):
    ...
    file = models.FileField(
        upload_to='gallery/',
        validators=[validate_video_file] # This will only run for video files
    )
    thumbnail = models.ImageField(upload_to='gallery/thumbnails/', blank=True)
    categories = models.ManyToManyField(GalleryCategory, related_name='media_items')
    ...
    def clean(self):
        if self.media_type == 'video' and not any(self.file.name.lower().endswith(ext) for ext in ['.mp4', '.webm', '.ogg']):
            raise ValidationError('Video files must be in MP4, WebM, or OGG format.')
